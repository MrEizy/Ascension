...
public record RemoveFormationFeature(
        Identifier formation
) implements SkillExecutionFeature {
    public static final MapCodec<RemoveFormationFeature> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
            Identifier.CODEC.fieldOf("formation").forGetter(RemoveFormationFeature::formation)
    ).apply(instance, RemoveFormationFeature::new));

    @Override
    public SkillExecutionFeatureType getType() {
        return AscensionSkillExecutionFeatureTypes.REMOVE_FORMATION.get();
    }

    @Override
    public void apply(SkillExecutionContext context) {
        FormationService.removeOwned(context.level(), context.caster().getUUID(), formation);
    }
}
