...
public final class DatapackRuntimeVisualController implements RuntimeVisualController {
    ...
    public void tick(RuntimeVisualState state) {
        ...
        RuntimeVisualDefinition definition = definition(state.visual());
        if (definition == null) {
            return;
        }
    ...
    private void tickDefinition(
        ...
        if (state.visual() != null && !visited.add(state.visual())) {
            return;
        }
        double time = level.getGameTime() + (state.seed() & 1023L);
        for (RuntimeVisualDefinition.Layer layer : definition.layers()) {
            if (layer.primitive() == RuntimeVisualDefinition.Primitive.PARTICLE_EMITTER) {
                emitParticles(layer, state, level, time);
            } else if (layer.primitive() == RuntimeVisualDefinition.Primitive.COMPOSITE) {
                layer.resources().visual().ifPresent(id -> {
                    RuntimeVisualDefinition nested = definition(id);
                    if (nested != null) {
                        tickDefinition(nested, withVisual(state, id), level, visited);
                    }
                });
            }
        }
        if (state.visual() != null) {
            visited.remove(state.visual());
        }
    }
    ...
    private void renderDefinition(
        ...
        double time = minecraft.level.getGameTime() + (state.seed() & 1023L);
        for (RuntimeVisualDefinition.Layer layer : definition.layers()) {
    ...
    private void renderLayer(
        ...
        if (layer.primitive() == RuntimeVisualDefinition.Primitive.MODEL_LAYER) {
            layer.resources().visual().ifPresent(id -> {
                RuntimeVisualController controller = ClientRuntimeVisuals.registeredController(id);
                if (controller != null && controller != this) {
                    controller.render(withVisual(state, id), event);
                }
            });
            return;
        }
        if (layer.primitive() == RuntimeVisualDefinition.Primitive.COMPOSITE) {
            layer.resources().visual().ifPresent(id -> {
                RuntimeVisualDefinition nested = definition(id);
                if (nested != null) {
                    renderDefinition(nested, withVisual(state, id), event, visited);
                }
            });
            return;
        }
        ...
        List<Vec3> centers = centers(layer, state, time);

        switch (layer.primitive()) {
            case BILLBOARD -> {
                for (Vec3 center : centers) {
                    drawBillboard(buffers, pose, center.subtract(camera), camera.subtract(center), width, height, color,
            ...
            case RING -> {
                for (Vec3 center : centers) {
                    drawRing(buffers, pose, center.subtract(camera), radius, innerRadius, layer.geometry().segments(),
                            rotation, color, layer.appearance());
                }
            }
            case SHELL -> {
                for (Vec3 center : centers) {
                    drawShell(buffers, pose, center.subtract(camera), radius, height, layer.geometry().segments(),
                            rotation, color, layer.appearance());
                }
                buffers.endLastBatch();
            }
            case BEAM -> {
                ...
                drawBeams(lines, pose, state, camera, layer, length, color);
                buffers.endLastBatch();
            }
            case GROUND_GLYPH -> {
                for (Vec3 center : centers) {
                    drawGroundGlyph(buffers, pose, center.subtract(camera), radius, innerRadius,
                            layer.geometry().segments(), rotation, color, layer);
                }
                buffers.endLastBatch();
            }
            case RIBBON -> {
                VertexConsumer lines = lineBuffer(buffers, layer.appearance().noDepth());
                drawRibbon(lines, pose, state, camera, color, layer.appearance().lineWidth(), layer.motion().history());
                buffers.endLastBatch();
            }
            case AFTERIMAGE -> {
                drawAfterimages(buffers, pose, state, camera, width, height, color, layer.resources().texture().orElse(null),
                        layer.motion().history());
                buffers.endLastBatch();
            }
            case LIVING_ENTITY_OVERLAY -> {
                for (Vec3 center : centers) {
                    drawEntityOverlay(buffers, pose, center.subtract(camera), radius, height,
                            layer.geometry().segments(), rotation, color, layer.appearance());
                }
                buffers.endLastBatch();
            }
            default -> {
    ...
    private void emitParticles(
        ...
        for (Vec3 center : centers(layer, state, time)) {
    ...
    private List<Vec3> centers(RuntimeVisualDefinition.Layer layer, RuntimeVisualState state, double time) {
        List<Vec3> values = new ArrayList<>();
        if (layer.position() == RuntimeVisualDefinition.PositionMode.EACH_POINT && !state.points().isEmpty()) {
            values.addAll(state.points());
        } else if (layer.position() == RuntimeVisualDefinition.PositionMode.OWNER) {
            Minecraft minecraft = Minecraft.getInstance();
            Player owner = state.ownerId() == null || minecraft.level == null
                    ? null
                    : minecraft.level.getPlayerByUUID(state.ownerId());
            values.add(owner == null
                    ? ClientRuntimeVisuals.position(state)
                    : owner.position().add(0.0D, owner.getBbHeight() * 0.5D, 0.0D));
        } else {
            values.add(ClientRuntimeVisuals.position(state));
        }
    ...
    private void drawRing(
        ...
        if (appearance.filled() && radius > 0.0D) {
            ...
            for (int index = 0; index < segments; index++) {
                ...
                Vec3 outerFirst = transform(new Vec3(Math.cos(first) * radius, 0.0D, Math.sin(first) * radius), rotation).add(center);
                Vec3 outerSecond = transform(new Vec3(Math.cos(second) * radius, 0.0D, Math.sin(second) * radius), rotation).add(center);
                Vec3 innerSecond = transform(new Vec3(Math.cos(second) * inner, 0.0D, Math.sin(second) * inner), rotation).add(center);
                Vec3 innerFirst = transform(new Vec3(Math.cos(first) * inner, 0.0D, Math.sin(first) * inner), rotation).add(center);
        ...
        for (int index = 0; index < segments; index++) {
            double first = Math.PI * 2.0D * index / segments;
            double second = Math.PI * 2.0D * (index + 1) / segments;
            Vec3 start = transform(new Vec3(Math.cos(first) * radius, 0.0D, Math.sin(first) * radius), rotation).add(center);
            Vec3 end = transform(new Vec3(Math.cos(second) * radius, 0.0D, Math.sin(second) * radius), rotation).add(center);
    ...
    private void drawBeams(
        ...
        Vec3 start = ClientRuntimeVisuals.position(state).add(offset);
        Vec3 direction = state.offset().lengthSqr() <= 1.0E-8D ? new Vec3(0.0D, 1.0D, 0.0D) : state.offset().normalize();
        line(lines, pose, start.subtract(camera), start.add(direction.scale(length)).subtract(camera), color,
                layer.appearance().lineWidth());
    }
    ...
    private void drawGroundGlyph(
        ...
        if (texture != null) {
            ...
            texturedQuad(textured, pose, center.subtract(right).subtract(forward), center.add(right).subtract(forward), center.add(right).add(forward), center.subtract(right).add(forward), color, new Vec3(0.0D, 1.0D, 0.0D));
            return;
        }
        ...
        for (int index = 0; index < spokes; index++) {
            ...
            Vec3 start = transform(new Vec3(Math.cos(angle) * inner, 0.0D, Math.sin(angle) * inner), rotation).add(center);
            Vec3 end = transform(new Vec3(Math.cos(angle) * radius, 0.0D, Math.sin(angle) * radius), rotation).add(center);
    ...
    private void drawRibbon(
        ...
        List<Vec3> points = history(state, limit);
        for (int index = 1; index < points.size(); index++) {
            float fraction = index / (float) points.size();
            RuntimeVisualDefinition.VisualColor faded = color.withAlpha(Math.max(1, Math.round(color.alpha() * fraction)));
            line(lines, pose, points.get(index - 1).subtract(camera), points.get(index).subtract(camera), faded, width);
        }
    }
    ...
    private void drawAfterimages(
        ...
        for (int index = 0; index < points.size(); index += 2) {
            ...
            RuntimeVisualDefinition.VisualColor faded = color.withAlpha(Math.max(1, Math.round(color.alpha() * fraction * 0.7F)));
            Vec3 center = points.get(index);
            drawBillboard(buffers, pose, center.subtract(camera), camera.subtract(center), width, height, faded, texture);
        }
    }
    ...
    private void drawEntityOverlay(
        ...
        drawShell(buffers, pose, center, Math.max(0.2D, radius), Math.max(0.4D, height),
                Math.max(8, segments), rotation, softened, appearance);
    }
    ...
    private void drawBillboard(
        ...
        Vec3 forward = toCamera.lengthSqr() <= 1.0E-8D ? new Vec3(0.0D, 0.0D, 1.0D) : toCamera.normalize();
    ...
    private Vec3 shellPoint(double radius, double height, double latitude, double longitude, Vec3 rotation) {
        double cos = Math.cos(latitude);
        return transform(new Vec3(
    ...
    private Vec3 transform(Vec3 value, Vec3 degrees) {
        ...
        if (degrees.y != 0.0D) {
            ...
            double cos = Math.cos(angle);
            double sin = Math.sin(angle);
            result = new Vec3(result.x * cos - result.z * sin, result.y, result.x * sin + result.z * cos);
        }
        if (degrees.z != 0.0D) {
    ...
    private void remember(RuntimeVisualState state, int limit) {
        Vec3 position = ClientRuntimeVisuals.position(state);
    ...
    private List<Vec3> history(RuntimeVisualState state, int limit) {
        ...
        if (values == null || values.isEmpty()) {
            return List.of(ClientRuntimeVisuals.position(state));
        }
    ...
    private int historyLimit(RuntimeVisualState state) {
        RuntimeVisualDefinition definition = state.visual() == null ? null : definition(state.visual());
    ...
    private RuntimeVisualDefinition definition(Identifier id) {
        ...
        return CoreRegistries.safeAccess(CoreRegistries.RUNTIME_VISUAL_REGISTRY, id, minecraft.level.registryAccess());
    }
